...
for keywordsArr in data['Index Keywords']:
    for eachWord in keywordsArr:
        keywords.append(eachWord.strip())
...
papers=pd.DataFrame({'papers' : papers}).drop_duplicates()        
keywords=pd.DataFrame({'keywords' : keywords}).drop_duplicates()
...
#Adding all keywords in users_keywords matrix
for index, row in keywords.iterrows():
    users_keywords[row[0]]=0

#Adding all papers id in users_papers matrix
...
#Randomly assign rating to the papers
for index, row in users_papers_ratings.iterrows():
    row=row.apply(assign_ratings)
    users_papers_ratings.loc[index]=row

#Storing all papers id into paper index list
